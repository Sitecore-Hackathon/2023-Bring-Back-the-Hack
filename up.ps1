...
function Invoke-NpmInit {	
    ...
    Write-Host "Running npm install in $Folder" -ForegroundColor Green
...
$topologyArray = @("xm1-sxa") # "xp0", "xp1", "xm1"; # xm1-sxa
...
Invoke-NpmInit './src/Sites/styleguide/nextjs'
Invoke-NpmInit './src/Sites/www/nextjs'


if (-not $envCheck) {
    throw "$envCheckVariable does not have a value. Did you run 'init.ps1 -InitEnv'?"
}
Write-Host "Entering $workingDirectoryPath" -ForegroundColor Green
...
# Ensure there is no env license value
...
if ($ByPass) {
  dotnet sitecore login --cm https://cm.hackback.localhost/ --auth https://id.hackback.localhost/ --allow-write true --client-id "SitecoreCLIServer" --client-secret "testsecret" --client-credentials true
}else {
  dotnet sitecore login --cm https://cm.hackback.localhost/ --auth https://id.hackback.localhost/ --allow-write true
}
...
Start-Process https://cm.hackback.localhost/sitecore/
Start-Process https://www.hackback.localhost/
